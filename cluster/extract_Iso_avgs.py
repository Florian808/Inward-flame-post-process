...
parser.add_argument("-T", "--Temperature", nargs="+", help="Define Isotherm temperature(s); Default T=3")
parser.add_argument("-e", "--extended", action='store_true', help="Enalbe extended scalars from post-processing script")
parser.add_argument("-l", "--local", action='store_true', help="Disable slurm commands to run script locally")
...
Tiso = 3              # Temperature used for isocountour unless specified in argument

# Start parallel compute engine
if not (test_mode or args.local):
     n_proc = 1
...
for path, dest in zip(args.sources, args.destinations):
    ...
    dB.db_type = "Xmdv"
    if args.po:
        dB.variables = ("x_velocity","y_velocity","temperature", "s1", "s2", "s3", "s4", "s5", "s6", "7", "s8", "s9",
                        "s10", "s11", "s12", "s13", "s14", "s15", "s16", "17", "s18", "s19", "s20", "s21")
